...
def parse_args():
    ...
    parser.add_argument('--l_steps', type=int, default=40, help='number of langevin steps')
    parser.add_argument('--l_step_size', type=float, default=0.1, help='stepsize of langevin')
    parser.add_argument('--sigma', type=float, default=0.8, help='prior of llhd, annealing parameter')
...
def predict(args, output_dir, device='cpu'):
    ...
    model.eval()
    z = 10 * torch.randn([81, args.nz])
    index = np.ceil(np.random.rand(2)*args.nz).astype(np.int16)
    k = 0
    for i in np.linspace(-10, 10, num=9, endpoint=False):
        for j in np.linspace(-10, 10, num=9, endpoint=False):
...
def main():
    ...
    # predict(args, os.path.join(exp_id, 'warm_start'), device)
