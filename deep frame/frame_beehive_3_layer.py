...
def opts(output_dir):
    ...
    args.num_chains = 1
    args.sigma = 1.0
    args.langevin_step_num = 100
    # TODO tune step size
    args.langevin_step_size = 0.1
    # TODO tune learning rate
    args.lr = [0.005, 0.003, 0.0005, 0.0003, 0.000001, 0.0000005]
    args.beta1 = 0.5
...
class Descriptor(nn.Module):
    def __init__(self):
        ...
        self.conv1 = models.vgg16(pretrained=True).features._modules['0']

        # self.conv2 = models.vgg16(pretrained=True).features._modules['2']
        # self.conv3 = models.vgg16(pretrained=True).features._modules['5']

        # self.conv1 = nn.Conv2d(3, 100, 15, 1, 7, bias=True)
    ...
    def train(self):
        ...
        for epoch in range(self.opts.num_epoch):
            ...
            if epoch % 10 == 0:
                ...
                save_images(rescaleSynthesizedImage(     + im_mean), '{}/{}.png'.format(self.opts.output_dir, epoch))
...
def main():
    ...
    test_multi_pic('./output/frame_beehive_3_layer/onelayer/ckpt', 'descriptor_2000.pth', 'cuda')
...
def test_multi_pic(dir, filename, device):
    import cv2
    descriptor = loadModel(dir, filename).to(device=device)
    filters = list(descriptor.parameters())[0].data.to(device=device)
    data_path = '../description_tex/image'
    ...
    for i in images:
        ...
        for j in filters:
            j = j.unsqueeze(0)
            print(j.shape)
            new_image = torch.nn.functional.conv2d(image, weight=j, bias=None, stride=1, padding=0, dilation=1, groups=1)
            ans += new_image
        vutils.save_image(ans, os.path.join(data_path, '{}_onelayer_res.png'.format(index)), normalize=True)
